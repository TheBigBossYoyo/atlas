...
export type PageViewProps = {
  ...
  /**
   * D23-PERF — `collectRunMetaByParagraph`/`collectBookmarkNamesByParagraph`
   * each walk the WHOLE document, not just this page. Computing them once
   * per document in `PageStack` (which knows every page shares the same
   * `document`/`relationships`) and passing the result down turns an
   * O(pageCount × documentSize) cost — every page re-walking the entire
   * document on every render — into O(documentSize). Left optional (falling
   * back to computing them locally below) so a `PageView` rendered on its
   * own — every test in `PageView.test.tsx`/`PageView.drawing.test.tsx`, and
   * any future standalone caller — keeps working unchanged.
   */
  runMetaByParagraph?: ReadonlyMap<string, ReadonlyArray<RunMeta | undefined>>;
  bookmarkNamesByParagraph?: ReadonlyMap<string, ReadonlyArray<string>>;
};
...
const PageViewComponent: React.FC<PageViewProps> = ({
  ...
  relationships,
  onResizeTableColumn,
  runMetaByParagraph: runMetaByParagraphProp,
  bookmarkNamesByParagraph: bookmarkNamesByParagraphProp,
}) => {
  const scale = zoom * PT_TO_PX;
  const resolvedRelationships = relationships ?? EMPTY_RELATIONSHIPS;
  // D23-PERF — `??` short-circuits, so `collectRunMetaByParagraph`/
  // `collectBookmarkNamesByParagraph` (each an O(documentSize) walk) only
  // actually run here when the caller didn't already hoist them — see the
  // props' own doc comment.
  const runMetaByParagraph = useMemo(
    () => runMetaByParagraphProp ?? collectRunMetaByParagraph(document, resolvedRelationships),
    [runMetaByParagraphProp, document, resolvedRelationships],
  );
  const bookmarkNamesByParagraph = useMemo(
    () => bookmarkNamesByParagraphProp ?? collectBookmarkNamesByParagraph(document),
    [bookmarkNamesByParagraphProp, document],
  );
...
// D23-PERF — a page's own props (`page`, `document`, etc.) are the only
// thing its rendered output depends on; wrapping in `memo` lets an unrelated
// parent re-render (e.g. `PageStack` re-rendering because a SIBLING page's
// props changed) skip this component entirely when none of ITS OWN props
// changed by reference. Default shallow-equal `memo` comparison is exactly
// right here — every prop is either a primitive, or an object `PageStack`
// only replaces when its own content actually changed (see `pagesDocument`
// and `runMetaByParagraph`/`bookmarkNamesByParagraph` in DocxViewer.tsx/
// PageStack.tsx).
