 ...
 *
 * This was written against a sibling branch (docx-fields-fonts) that, at
 * the time, hadn't yet parsed `w:fldChar`/`w:instrText`/`w:fldSimple` into
 * a real model node — deliberately NOT duplicated here to avoid two
 * branches racing to parse the same OOXML shape differently.
 *
 * Now that both have landed on main: `docx-fields-fonts` did build its own
 * Field model and its own `evaluate.ts` PAGE/NUMPAGES evaluators, wired
 * into `DocxViewer.tsx`'s "Update Fields" — but its evaluators originally
 * ignored the `\* ROMAN`/`\* roman`/`\* ALPHABETIC`/`\* alphabetic`
 * numeric-format switch entirely. Rather than leaving this module as dead,
 * never-referenced code, `evaluate.ts`'s `evaluateNumPagesField`/
 * `evaluatePageField` now delegate their numeric formatting to
 * `resolvePageField` below — the one piece of this problem that belongs to
 * layout rather than parsing: turning an already-extracted instruction
 * string (`"PAGE"`, `"NUMPAGES \* ROMAN"`, `"SECTIONPAGES"`, ...) into
 * resolved display text, independent of any particular Field/context
 * model shape. `SECTIONPAGES` itself still has no corresponding `FieldType`
 * in `docx/model` (only `PAGE`/`NUMPAGES` are modeled today), so this
 * resolver's `SECTIONPAGES` branch remains reachable only from its own
 * unit tests until a future field-type addition wires it up.
 */
