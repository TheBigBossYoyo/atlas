...
// same require.cache-injection trick `mainIpcTestHarness.ts`'s `loadMainCjs`
// uses for `electron` itself, applied to a second local module via its
// `extraModules` parameter) so the `export:printToPdf` HANDLER's own
